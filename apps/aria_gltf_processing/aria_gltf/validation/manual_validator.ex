...
defmodule AriaGltf.Validation.ManualValidator do
  @moduledoc """
  Manual validation logic for glTF 2.0 JSON schema compliance.

  This module implements pure Elixir validation for critical constraints
  that must always be checked, independent of JSON schema validation.
  """

  alias AriaGltf.Validation.Context

  @doc """
  Validates a JSON document using manual validation logic.

  Performs comprehensive structural validation following glTF 2.0 specification
  including required fields, type checks, bounds validation, and constraints.
  """
  @spec validate(Context.t(), map()) :: Context.t()
  def validate(context, json) when is_map(json) do
    errors = []
